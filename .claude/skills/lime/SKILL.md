...
name: lime-ui
description: >-
  Build apps with the Lime UI kit. Use whenever the user asks to create, extend,
  or style any UI in a project that contains this kit. Bold marketing kit — big
  type, lime, cream + ink, grotesk display, monospace labels, highlight-marker
  headings, dark mode, and EN/AR + RTL. Gives exact components, tokens, and rules.
---

# Building with Lime

Lime is a bold marketing kit: cream + ink + vivid lime, a grotesk display face,
...
2. **Type roles.** Headings → `font-display` (Hanken Grotesk) and wrap the key word in
